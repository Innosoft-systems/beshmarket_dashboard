            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b last:border-0 hover:bg-muted/30 transition-colors flex items-start gap-3 ${!n.is_read ? "bg-blue-50/50" : ""}`}
                >
                  ...
                  {n.data?.link && <ExternalLink className="h-3.5 w-3.5 text-muted-foreground shrink-0 mt-0.5" />}
                </button>
              ))
            )}
          </div>
